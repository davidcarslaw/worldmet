...
#' @noRd
deprecate_isd <- function(newfun) {
  warning(
    c(
      "The Integrated Surface Database (ISD) has been deprecated by NOAA in favour of the Global Historical Climatology Network hourly (GHCNh). Please use `",
      newfun,
      "()`."
    )
  )
...
#'   a range of years e.g. 1990:2016 (which would dplyr::select any site that had an
...
importNOAA <- function(code = "037720-99999",
  ...
  ## go through each of the years dplyr::selected, use parallel processing
  ...
  if (n.cores > 1) {
    ...
    dat <-
      purrr::pmap(site_process,
        getDat,
        hourly = hourly,
        .progress = "Importing NOAA Data"
      ) %>%
      purrr::list_rbind()
  }
  ...
  actual <- dplyr::select(dat, "code", "date", "station") %>%
    ...
    dplyr::group_by(.data$code, .data$year) %>%
    dplyr::slice(1)

  actual <-
    dplyr::left_join(site_process, actual, by = c("code", "year"))
...
getDat <- function(code, year, hourly) {
  ...
  met_data <- try(suppressWarnings(readr::read_csv(
    file.name,
  ...
  if ("WND" %in% names(met_data)) {
    met_data <-
      tidyr::separate(met_data, "WND", into = c("wd", "x", "y", "ws", "z"))
  ...
  if ("VIS" %in% names(met_data)) {
    ...
    met_data <- dplyr::mutate(
      ...
      visibility = ifelse(.data$visibility %in% c(9999, 999999), NA, .data$visibility)
    )
  }
  ...
  met_data$RH <-
    100 * ((112 - 0.1 * met_data$air_temp + met_data$dew_point) /
  ...
  if ("GA1" %in% names(met_data)) {
    ...
    met_data <- dplyr::mutate(
      ...
      cl_1 = ifelse((is.na(.data$cl_1) &
        .data$ceil_hgt == 22000), 0, .data$cl_1),
  ...
  if ("cl_3" %in% names(met_data)) {
    met_data$cl <-
      pmax(met_data$cl_1, met_data$cl_2, met_data$cl_3, na.rm = TRUE)
  }
  ...
  if ("AW1" %in% names(met_data)) {
    ...
    met_data <-
      dplyr::left_join(met_data, worldmet::weatherCodes, by = "pwc")
    met_data <- dplyr::select(met_data, -"pwc") %>%
      dplyr::rename(pwc = "description")
  }

  ## dplyr::select the variables we want
  met_data <- dplyr::select(met_data, dplyr::any_of(
    c(
      "date",
      "code",
      "station",
  ...
  if ("pwc" %in% names(met_data) && hourly) {
    ...
    dates <-
      as.POSIXct(paste0(unique(pwc$date2), ":00:00"), tz = "GMT")
  ...
  if ("precip_12" %in% names(met_data) && hourly) {
    ...
    for (i in seq_along(id)) {
      met_data$precip[(id[i] - 11):id[i]] <-
        met_data$precip_12[id[i]] / 12
    }
  }
